...
type ApplicationTrust = { completionPercent: number; canApply: boolean; requiredMissing: string[]; isMockAccount: boolean };

function ApplicationLifestylePhoto({ eventId, onUploaded }: { eventId: string | number; onUploaded: (url: string | null) => void }) {
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [uploading, setUploading] = useState(false);
  const [error, setError] = useState("");
  const upload = async (file: File) => {
    if (!["image/jpeg", "image/png", "image/webp"].includes(file.type)) { setError("僅支援 JPG、PNG 或 WebP 生活照。"); return; }
    if (file.size > 5 * 1024 * 1024) { setError("生活照請小於 5MB。"); return; }
    setError(""); setUploading(true); setPreviewUrl(URL.createObjectURL(file));
    const formData = new FormData(); formData.set("photo", file);
    const response = await fetch(`/api/events/${eventId}/application-photo`, { method: "POST", body: formData }).catch(() => null);
    const payload = await response?.json().catch(() => ({})) as { photoUrl?: string; error?: string };
    setUploading(false);
    if (!response?.ok || !payload.photoUrl) { setError(payload.error === "UNAUTHENTICATED" ? "請先登入後再上傳生活照。" : "生活照上傳未完成，請稍後再試。"); setPreviewUrl(null); onUploaded(null); return; }
    setPreviewUrl(payload.photoUrl); onUploaded(payload.photoUrl);
  };
  return <div className="mt-3 rounded-2xl border border-amber-100 bg-amber-50/70 p-3"><p className="text-xs font-black text-amber-950">生活照／氛圍照 <span className="font-medium text-amber-700">（可略過）</span></p><p className="mt-1 text-[11px] leading-relaxed text-amber-800">這不是頭像，僅會顯示給本場飯局的主辦人，協助對方在審核時更認識你的生活風格。請上傳自己的照片，勿含他人敏感資訊。</p><div className="mt-3 flex items-center gap-3">{previewUrl ? <img src={previewUrl} alt="生活照預覽" className="h-16 w-16 rounded-xl object-cover" /> : <div className="grid h-16 w-16 place-items-center rounded-xl bg-white text-[10px] font-bold text-amber-600">尚未上傳</div>}<label className="pressable flex-1 cursor-pointer rounded-xl bg-white px-3 py-2.5 text-center text-xs font-bold text-amber-800 shadow-sm"><input aria-label="上傳生活照" type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); }} />{uploading ? "生活照上傳中…" : previewUrl ? "更換生活照" : "選擇生活照"}</label>{previewUrl && <button type="button" onClick={() => { setPreviewUrl(null); onUploaded(null); }} className="pressable rounded-xl bg-white px-3 py-2.5 text-xs font-bold text-slate-600">移除</button>}</div>{error && <p role="alert" className="mt-2 text-[11px] font-bold text-rose-700">{error}</p>}</div>;
}
