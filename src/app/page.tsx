      ...
      <button onClick={onOpen} className="block w-full text-left" aria-label={`查看 ${event.title} 詳情`}>
        <div className={`relative h-32 overflow-hidden rounded-[19px] bg-gradient-to-br ${event.color} p-4 text-white`}>
          ...
          <div className="absolute bottom-3 left-4 right-4 flex items-end justify-between gap-2">
            ...
            <span className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-full bg-white/20 text-xs font-black backdrop-blur-sm">{event.hostAvatarUrl ? <img src={event.hostAvatarUrl} alt={`${event.host} 的頭像`} className="h-full w-full object-cover" /> : event.hostInitial}</span>
          </div>
        </div>
      </button>
...
function ExplorePage({ events, notice, onOpen, userName, userAvatarUrl, onAccountClick }: { events: DiningEvent[]; notice: string; onOpen: (event: DiningEvent) => void; userName?: string | null; userAvatarUrl?: string | null; onAccountClick: () => void }) {
    ...
    <section className="page-enter app-top-safe px-4 pb-28">
      <header className="relative overflow-hidden rounded-[30px] bg-slate-950 px-5 pb-6 pt-5 text-white shadow-[0_20px_45px_rgba(27,12,62,0.28)]">
        ...
        <div className="relative flex items-center justify-between">
          {userName ? <button type="button" onClick={onAccountClick} aria-label={`開啟 ${userName} 的個人主頁`} className="pressable flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-2 text-xs font-bold backdrop-blur-md">{userAvatarUrl ? <img src={userAvatarUrl} alt="" className="h-4 w-4 rounded-full object-cover" /> : <MapPin className="h-3.5 w-3.5 text-orange-300" />}{userName}</button> : <a href="/api/auth/login" className="flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-2 text-xs font-bold backdrop-blur-md"><MapPin className="h-3.5 w-3.5 text-orange-300" />登入／台北市</a>}
  ...
  return (
    <section className="page-enter flex min-h-[680px] flex-col px-4 pb-28 pt-5">
      ...
      <div className="mt-5 flex flex-1 flex-col overflow-hidden rounded-[28px] border border-white/70 bg-white/78 shadow-[0_18px_45px_rgba(55,28,98,0.13)] backdrop-blur-xl">
        <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-3.5"><div className="grid h-10 w-10 place-items-center overflow-hidden rounded-2xl bg-gradient-to-br from-fuchsia-500 to-violet-600 text-sm font-black text-white">{messages.find((item) => item.author.id !== currentUserId)?.author.avatarUrl ? <img src={messages.find((item) => item.author.id !== currentUserId)?.author.avatarUrl || ""} alt="聊天室成員頭像" className="h-full w-full object-cover" /> : (chatTarget?.memberName || "M").slice(0, 1)}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-slate-900">{chatTarget?.eventTitle || "週五夜的微醺義式晚餐"}</p><p className="mt-0.5 text-xs font-medium text-emerald-600">{chatTarget ? `飯局群組聊天室 · 可傳訊給 ${chatTarget.memberName}` : "3 位已確認成員 · 可安心聊天"}</p></div><MoreHorizontal className="h-5 w-5 text-slate-400" /></div>
...
function ChatBubble({ name, text, own, inverse, avatarUrl }: { name: string; text: string; own?: boolean; inverse?: boolean; avatarUrl?: string | null }) {
  ...
  return <div className={`flex items-end gap-2 ${isOwn ? "justify-end" : ""}`}>{!isOwn && <span className="grid h-7 w-7 shrink-0 place-items-center overflow-hidden rounded-full bg-violet-100 text-[10px] font-black text-violet-700">{avatarUrl ? <img src={avatarUrl} alt={`${name} 的頭像`} className="h-full w-full object-cover" /> : name.slice(0, 1)}</span>}<div className={`max-w-[82%] ${isOwn ? "order-1" : ""}`}><p className={`mb-1 text-[10px] font-bold ${isOwn ? "text-right text-violet-500" : "text-slate-400"}`}>{name}</p><p className={`rounded-2xl px-3 py-2.5 text-sm leading-relaxed ${isOwn ? "rounded-tr-sm bg-violet-600 text-white" : "rounded-tl-sm bg-slate-100 text-slate-700"}`}>{text}</p></div></div>;
}
