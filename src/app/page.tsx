...
function eventFromApi({ event, host }: ApiEventRow): DiningEvent {
  ...
  return { id: event.id, title: event.title, cuisine: event.cuisineTags?.join(" · ") || "新建立飯局", date: new Intl.DateTimeFormat("zh-TW", { timeZone: TAIPEI_TIME_ZONE, month: "numeric", day: "numeric", weekday: "short" }).format(startsAt), time: formatTaipeiTime(startsAt), restaurant: event.restaurantName || event.venueAddress, venueAddress: event.venueAddress, neighborhood: event.neighborhood || "地點待確認", capacity: `最多 ${event.capacity} 人`, payment, paymentShort: payment === "各自付" ? "AA 制" : payment, budget, host: host.displayName, hostInitial: host.displayName.slice(0, 1) || "飯", color: "from-emerald-500 via-teal-500 to-cyan-400", accent: "bg-emerald-500", lat: Number(event.latitude) || 25.0339, lng: Number(event.longitude) || 121.5645 };
}
...
type ChatMessageRecord = { message: { id: string; content: string; createdAt: string }; author: { id: string; displayName: string } };

function MessagesPage({ chatTarget, currentUserId }: { chatTarget?: { eventId: string; eventTitle: string; memberName: string } | null; currentUserId?: string | null }) {
  ...
  return (
    <section className="page-enter flex min-h-[680px] flex-col px-4 pb-28 pt-5">
      ...
      <div className="mt-5 flex flex-1 flex-col overflow-hidden rounded-[28px] border border-white/70 bg-white/78 shadow-[0_18px_45px_rgba(55,28,98,0.13)] backdrop-blur-xl">
        ...
        <div className="hide-scrollbar flex-1 space-y-4 overflow-y-auto px-4 py-5">{!chatTarget ? <div className="rounded-2xl border border-dashed border-violet-200 bg-violet-50 p-4 text-center text-xs leading-relaxed text-violet-800">請從「我的飯局」的已確認成員卡片點選「傳送訊息」，開啟對應飯局的群組聊天室。</div> : loading ? <p className="text-center text-xs font-bold text-slate-400">正在載入真實歷史訊息…</p> : chatError ? <p role="alert" className="rounded-2xl bg-rose-50 p-3 text-xs font-bold leading-relaxed text-rose-700">{chatError}</p> : <>{messages.length === 0 && <p className="rounded-2xl bg-slate-50 p-3 text-center text-xs text-slate-500">這個飯局尚無訊息，發送第一則問候吧。</p>}{messages.map(({ message: item, author }) => <ChatBubble key={item.id} name={author.id === currentUserId ? "你" : author.displayName} text={item.content} own={author.id === currentUserId} />)}</>}<div className="rounded-2xl border border-orange-100 bg-orange-50 px-3 py-2.5 text-[11px] leading-relaxed text-orange-700"><ShieldCheck className="mr-1 inline h-3.5 w-3.5" />請勿在聊天室要求匯款或分享敏感資訊；若有疑慮，可立即檢舉。</div></div>
...
function ProfileV2({ onOpenPrd, onOpenMyEvents, userName, onLogout, isAdmin, onOpenAdmin, verificationStatus }: { onOpenPrd: () => void; onOpenMyEvents: () => void; userName?: string | null; onLogout: () => Promise<void>; isAdmin: boolean; onOpenAdmin: () => void; verificationStatus?: "unverified" | "pending" | "verified" | "rejected" | null }) {
    ...
    <section className="page-enter px-4 pb-28 pt-5">
      <div className="relative overflow-hidden rounded-[30px] bg-slate-950 p-5 text-white shadow-[0_20px_45px_rgba(27,12,62,0.28)]">
        ...
        <div className="relative flex items-start justify-between">
          <div className="flex gap-3"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-gradient-to-br from-orange-300 via-pink-500 to-violet-600 text-lg font-black">{initial}</div><div><p className="text-lg font-black">{displayName}</p><p className="mt-1 text-xs text-violet-200">建立你的第一筆飯局紀錄</p></div></div>
...
export default function Home() {
  ...
  const [sessionUser, setSessionUser] = useState<{ id?: string; displayName?: string | null; name?: string | null; role?: "member" | "moderator" | "admin"; verificationStatus?: "unverified" | "pending" | "verified" | "rejected" } | null>(null);
