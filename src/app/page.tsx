  ...
  return (
    <section className="page-enter flex min-h-[680px] flex-col px-4 pb-28 pt-5">
      ...
      <div className="mt-5 flex flex-1 flex-col overflow-hidden rounded-[28px] border border-white/70 bg-white/78 shadow-[0_18px_45px_rgba(55,28,98,0.13)] backdrop-blur-xl">
        <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-3.5"><div className="relative grid h-10 w-10 place-items-center overflow-hidden rounded-2xl bg-gradient-to-br from-fuchsia-500 to-violet-600 text-sm font-black text-white">{messages.find((item) => item.author.id !== currentUserId)?.author.avatarUrl ? <Image src={messages.find((item) => item.author.id !== currentUserId)?.author.avatarUrl || ""} alt={`${chatTarget?.memberName || "聊天室成員"} 的會員頭像`} fill sizes="40px" className="object-cover" /> : (chatTarget?.memberName || "M").slice(0, 1)}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-slate-900">{chatTarget?.eventTitle || "週五夜的微醺義式晚餐"}</p><p className="mt-0.5 text-xs font-medium text-emerald-600">{chatTarget ? `飯局群組聊天室 · 可傳訊給 ${chatTarget.memberName}` : "3 位已確認成員 · 可安心聊天"}</p></div><MoreHorizontal className="h-5 w-5 text-slate-400" /></div>
...
function ChatBubble({ name, text, own, inverse, avatarUrl }: { name: string; text: string; own?: boolean; inverse?: boolean; avatarUrl?: string | null }) {
  ...
  return <div className={`flex items-end gap-2 ${isOwn ? "justify-end" : ""}`}>{!isOwn && <span className="relative grid h-7 w-7 shrink-0 place-items-center overflow-hidden rounded-full bg-violet-100 text-[10px] font-black text-violet-700">{avatarUrl ? <Image src={avatarUrl} alt={`${name} 的會員頭像`} fill sizes="28px" className="object-cover" /> : name.slice(0, 1)}</span>}<div className={`max-w-[82%] ${isOwn ? "order-1" : ""}`}><p className={`mb-1 text-[10px] font-bold ${isOwn ? "text-right text-violet-500" : "text-slate-400"}`}>{name}</p><p className={`rounded-2xl px-3 py-2.5 text-sm leading-relaxed ${isOwn ? "rounded-tr-sm bg-violet-600 text-white" : "rounded-tl-sm bg-slate-100 text-slate-700"}`}>{text}</p></div></div>;
}
