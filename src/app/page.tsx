...
function eventFromApi({ event, host }: ApiEventRow): DiningEvent {
  ...
  return { id: event.id, title: event.title, cuisine: event.cuisineTags?.join(" · ") || "新建立飯局", date: new Intl.DateTimeFormat("zh-TW", { month: "numeric", day: "numeric", weekday: "short" }).format(startsAt), time: new Intl.DateTimeFormat("zh-TW", { hour: "2-digit", minute: "2-digit", hour12: false }).format(startsAt), restaurant: event.restaurantName || event.venueAddress, venueAddress: event.venueAddress, neighborhood: event.neighborhood || "地點待確認", capacity: `最多 ${event.capacity} 人`, payment, paymentShort: payment === "各自付" ? "AA 制" : payment, budget, host: host.displayName, hostInitial: host.displayName.slice(0, 1) || "飯", color: "from-emerald-500 via-teal-500 to-cyan-400", accent: "bg-emerald-500", lat: Number(event.latitude) || 25.0339, lng: Number(event.longitude) || 121.5645 };
}
...
export default function Home() {
  ...
  const [sessionUser, setSessionUser] = useState<{ displayName?: string | null; name?: string | null; role?: "member" | "moderator" | "admin"; verificationStatus?: "unverified" | "pending" | "verified" | "rejected" } | null>(null);
