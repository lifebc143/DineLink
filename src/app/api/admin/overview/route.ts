...
export async function GET() {
  ...
  const [registered, verified, eventsTotal, publishedEvents, applicationsTotal, pendingApplications, attendancesTotal, noShows, recentMembers] = await Promise.all([
    ...
    db.select({ id: users.id, displayName: users.displayName, role: users.role, verificationStatus: users.verificationStatus, createdAt: users.createdAt }).from(users).orderBy(desc(users.createdAt)).limit(6),
  ]);
