...
const inputSchema = z.object({
  introduction: z.string().trim().max(280).optional(),
  lifestylePhotoUrl: z.string().trim().max(500).optional(),
});
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  const trust = calculateProfileTrust(user);
  if (!trust.canApply) return NextResponse.json({ error: "PROFILE_INCOMPLETE", missingFields: trust.requiredMissing, completionPercent: trust.completionPercent }, { status: 409 });
  if (parsed.data.lifestylePhotoUrl && !parsed.data.lifestylePhotoUrl.includes(`application-lifestyle/${user.id}/${eventId}/`)) return NextResponse.json({ error: "INVALID_LIFESTYLE_PHOTO" }, { status: 400 });
  try {
    const response = await db.transaction(async (tx) => {
      ...
      const [application] = await tx.insert(eventApplications).values({ eventId, applicantId: user.id, introduction: parsed.data.introduction, lifestylePhotoUrl: parsed.data.lifestylePhotoUrl }).returning();
