...
export default function ProfileAvatarEditor({ user, onUpdated }: { user: AvatarUser; onUpdated: (updated: AvatarUser) => void }) {
  ...
  return <>
    ...
    {previewUrl && <div role="dialog" aria-modal="true" aria-labelledby="avatar-editor-title" data-testid="avatar-editor-fullscreen" className="fixed inset-0 z-[90] min-h-[100dvh] overflow-y-auto bg-[#fcfbff] px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-[max(1.25rem,env(safe-area-inset-top))] text-slate-900"><div className="mx-auto max-w-md"><div className="flex items-start justify-between"><div><p id="avatar-editor-title" className="text-xl font-black text-slate-950">調整會員頭像</p><p className="mt-1 max-w-[300px] text-xs leading-relaxed text-slate-500">預設會以中央圓形裁切；可調整縮放範圍，完成後將安全儲存。</p></div><button type="button" onClick={() => { URL.revokeObjectURL(previewUrl); setPreviewUrl(null); }} aria-label="關閉頭像裁切" className="pressable rounded-full bg-slate-100 p-2 text-slate-500"><X className="h-5 w-5" /></button></div><div className="mx-auto mt-6 grid h-36 w-36 place-items-center overflow-hidden rounded-full bg-slate-100 ring-4 ring-violet-100 shadow-sm sm:h-44 sm:w-44"><img src={previewUrl} alt="頭像裁切預覽" className="h-full w-full object-cover transition-transform duration-150" style={{ transform: `scale(${zoom})` }} /></div><label className="mx-auto mt-6 block max-w-md text-sm font-bold text-slate-700">縮放比例 <span className="text-violet-600">{zoom.toFixed(1)}×</span><input aria-label="調整頭像縮放比例" type="range" min="1" max="2.4" step="0.1" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} className="mt-3 w-full accent-violet-600" /></label><div data-testid="avatar-editor-actions" className="mx-auto mt-5 grid max-w-md grid-cols-2 gap-3"><button type="button" disabled={uploading} onClick={() => { URL.revokeObjectURL(previewUrl); setPreviewUrl(null); }} className="pressable rounded-xl bg-slate-100 py-3.5 text-sm font-bold text-slate-700 disabled:opacity-60">取消</button><button type="button" disabled={uploading} onClick={() => void upload()} className="pressable flex items-center justify-center gap-2 rounded-xl bg-violet-600 py-3.5 text-sm font-bold text-white shadow-[0_10px_20px_rgba(124,58,237,0.25)] disabled:opacity-60">{uploading ? <><LoaderCircle className="h-4 w-4 animate-spin" />上傳中…</> : <><Upload className="h-4 w-4" />套用頭像</>}</button></div></div></div>}
