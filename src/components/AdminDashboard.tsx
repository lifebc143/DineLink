...
const verificationLabel = (status: Member["verificationStatus"]) => ({ verified: "已驗證", pending: "待審核", rejected: "未通過", unverified: "未驗證" }[status]);
...
export default function AdminDashboard({ onBack }: { onBack: () => void }) {
  ...
  const memberActions = (member: Member) => {
    if (member.role === "admin") return <p className="mt-2 rounded-lg bg-violet-50 px-2.5 py-2 text-[10px] font-semibold leading-relaxed text-violet-700">管理員帳號受保護：不可由同一後台自行核准驗證、停權或取消資格；請由另一位管理員或依專案擁有者流程處理。</p>;
