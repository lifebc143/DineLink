...
describe("GET /api/me/events", () => {
  ...
  it("登入後回傳主辦飯局、待審核成員、確認成員與我已申請的飯局", async () => {
    ...
    const attendance = { attendance: { id: "55555555-5555-4555-8555-555555555555", eventId: hostedEvent.id, userId: "66666666-6666-4666-8666-666666666666", status: "confirmed" }, member: { id: "66666666-6666-4666-8666-666666666666", displayName: "小晴", avatarUrl: null } };
    ...
    mocks.select.mockReturnValueOnce(hostedQuery).mockReturnValueOnce(pendingQuery).mockReturnValueOnce(attendanceQuery).mockReturnValueOnce(appliedQuery);

    const response = await GET();
    const payload = await response.json();
    expect(response.status).toBe(200);
    expect(payload.hosted[0]).toMatchObject({ event: { id: hostedEvent.id, title: "我發起的飯局" }, pendingApplications: [{ applicant: { displayName: "小安" } }], attendances: [{ member: { displayName: "小晴" } }] });
