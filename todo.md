...
- [ ] 補齊建立成功後「我的飯局」可見的回歸測試，並在正式登入環境手動驗證後自動發佈。
- [ ] 補上登入後安全返回發起飯局頁的 OAuth 導向測試。
- [x] 修正發起飯局的 Google Maps／Places 地點搜尋、Autocomplete 選取與地圖切換互動。
- [x] 補上 Places 建議選取、座標解析與地圖開關的互動測試，完成地圖流程端到端驗證。
- [ ] 修正 POST /api/events、登入 Session 與 PostgreSQL 寫入後的成功回饋，確保新飯局可於我的飯局讀取。
- [ ] 補齊地點選取、地圖、建立寫入與清單同步的端到端回歸測試並重新自動發佈。
